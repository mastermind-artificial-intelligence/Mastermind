...
def givenum(num_colors, duplicate=False):
    """return a random combination from the full codeset"""
    if duplicate:
...
def playresult(secretcode, guess):
    """function to return feedback for each guess made"""
    A = 0
    B = 0
    temp_secretcode = []
    temp_guess = []
    
    for i in range(len(secretcode)): # checking for blacks only, and remove them from the list
        if secretcode[i] == guess[i]:
            A = A + 1      
        else:
            temp_secretcode.append(secretcode[i])
            temp_guess.append(guess[i])
    for guess in temp_guess: #checking for whites only
        if guess in temp_secretcode:
            B = B + 1 
            idx = temp_secretcode.index(guess)
            del temp_secretcode[idx]

    return A, B


def chooseone(code_set):
    """runs a look forward process based on referenced research paper to find the best guess that can reduce code set the most"""
    ...
    #returns the index of the lowest value in remain_table, this is the most optimal next guess
...
def start(code, duplicate=False, num_colors=5 ):
    ...
    while (A < 4):  # Do until guess is correct
